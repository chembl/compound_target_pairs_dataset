...
def remove_compounds_without_smiles_and_mixtures(df_combined: pd.DataFrame, chembl_con: sqlite3.Connection) -> pd.DataFrame:
    """
    Remove 
    ...
    smiles_with_dot = df_combined[df_combined['canonical_smiles'].notnull(
    ...
    for parent_molregno in set(smiles_with_dot['parent_molregno']):
        assert (len(df_hierarchy[df_hierarchy['parent_molregno'] == parent_molregno]) > 0), \
            f"The compound with the parent_moregno {parent_molregno} does not occur \
                as a parent_molregno in the molecule_hierarchy."
        df_salt_molregno = df_hierarchy[df_hierarchy['salt_molregno']
                                        == parent_molregno]
        assert (df_salt_molregno['salt_molregno'].equals(df_salt_molregno['parent_molregno'])), \
    ...
    for parent_molregno in set(smiles_with_dot['parent_molregno']):
        parent_smiles_in_chembl = df_parent_smiles[df_parent_smiles['parent_molregno']
                                                   == parent_molregno]['canonical_smiles'].item()
        parent_smiles_in_df = smiles_with_dot[smiles_with_dot['parent_molregno']
                                              == parent_molregno]['canonical_smiles'].item()
        assert (parent_smiles_in_chembl == parent_smiles_in_df), \
...
def reorder_columns(df_combined, calculate_RDKit):
    ...
    compound_target_pair_columns = ['parent_molregno', 'parent_chemblid', 'parent_pref_name',
    ...
    aggregated_values = ['pchembl_value_mean_BF', 'pchembl_value_max_BF', 'pchembl_value_median_BF',
    ...
    chembl_compound_props = ['mw_freebase', 'alogp', 'hba', 'hbd', 'psa',
    ...
    chembl_structures = ['standard_inchi',
                         'standard_inchi_key', 'canonical_smiles']
    ligand_efficieny_metrics = ['LE_B', 'BEI_B', 'SEI_B', 'LLE_B',
    ...
    if calculate_RDKit:
        columns = compound_target_pair_columns + \
            aggregated_values + DTI_annotations + first_publication_cpd + \
            chembl_compound_props + chembl_structures + \
    ...
    assert (len_columns_before ==
            len_columns_after), f"Different number of columns after reordering (before: {len_columns_before}, after: {len_columns_after})."
